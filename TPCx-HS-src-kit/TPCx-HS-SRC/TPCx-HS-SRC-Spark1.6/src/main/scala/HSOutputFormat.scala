...
class HSOutputFormat extends FileOutputFormat[Array[Byte], Array[Byte]] {
  ...
  def setFinalSync(job : JobContext, newValue : Boolean) =
    job.getConfiguration().setBoolean(HSOutputFormat.FINAL_SYNC_ATTRIBUTE, newValue);

  /**
   * Does the user want a final sync at close?
   */
  def getFinalSync(job : JobContext ) : Boolean =
    job.getConfiguration().getBoolean(HSOutputFormat.FINAL_SYNC_ATTRIBUTE, false);
  ...
  def getOutputPath(job : JobContext ) : Path =  {
    job.getConfiguration().get(HSOutputFormat.OUTDIR) match {
      case null => null
